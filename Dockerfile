...
WORKDIR /app/service
COPY leasebase-notification-service/package.json leasebase-notification-service/package-lock.json* ./
RUN npm ci
COPY leasebase-notification-service/tsconfig.json ./
COPY leasebase-notification-service/src ./src
RUN npm run build
